-- 리뷰받기 목록/상세의 상품·리뷰어 일괄입력용 canonical bulk RPC.
-- 한 함수 호출 전체가 하나의 PostgreSQL transaction이다. 50행 단위 처리는
-- 입력 검증·처리 경계를 위한 내부 batch 크기이며 chunk별 commit을 하지 않는다.
-- 따라서 어느 행에서 오류가 발생해도 상품·submission 전체가 rollback된다.
...
begin
  ...
  perform public.admin_gateway_validate_payload_keys(
    p_payload,
    array['groups', 'reusable_product_id']::text[]
  );
