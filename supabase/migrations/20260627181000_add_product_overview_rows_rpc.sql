create index if not exists products_manager_created_id_idx
  on public.products (manager_id, created_at desc, id asc);

create index if not exists submissions_product_status_created_id_idx
  on public.submissions (product_id, is_review_verified, is_deposit_verified, created_at, id);

create index if not exists evidence_photos_review_submission_idx
  on public.evidence_photos (submission_id)
  where photo_type = 'review';

create or replace function public.normalize_product_overview_filter_text(value text)
returns text
language sql
immutable
as $$
...
scoped_rows as (
  select
    ...
    coalesce(
      ...
      ) filter (where evidence_photos.id is not null),
      '[]'::jsonb
    ) as review_photos,
    count(evidence_photos.id)::integer as review_photo_count,
  ...
  left join public.evidence_photos
    on evidence_photos.submission_id = submissions.id
...
status_rows as (
  ...
  where normalized_inputs.status_key = 'all'
    or (
      ...
      and scoped_rows.is_review_verified = false
    )
    or (
      normalized_inputs.status_key = 'review'
      and scoped_rows.is_review_verified = true
      and scoped_rows.is_deposit_verified = false
    )
    or (
      normalized_inputs.status_key = 'complete'
      and scoped_rows.is_review_verified = true
      and scoped_rows.is_deposit_verified = true
    )
),
filtered_rows as (
  select status_rows.*
  ...
  where (
    ...
    and (
      ...
      or (
        ...
        and status_rows.review_photo_count > 0
      )
      or (
