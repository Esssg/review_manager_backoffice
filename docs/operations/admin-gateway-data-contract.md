...
상태: 로컬 구현·계약 초안과 production 부분 전환 상태. 2026-09-02 기준 production `vm-app-01`에는 상품/권한/성능 3건의 additive DB migration과 `admin-gateway` Edge Function을, `vm-web-01`에는 해당 backoffice app image를 반영했다. DB backup·RPC ACL·scope/filter/photo preview read 검증, Edge/web health 및 비인증 경계 검증을 완료했다. 실제 로그인 브라우저의 권한 canary와 2·30·100건 업무 데이터 등록 canary는 아직 수행하지 않았다.
...
현재 저장소의 기본값은 gateway 미활성 상태다. 기존 access/data/read/export migration들은 staging과 production 사전 점검 기준으로 관리하며, production에는 `20260829220000_fix_admin_review_receive_product_bundle_contract.sql`에 이어 `20260901120000_fix_admin_scope_filters_photo_bulk.sql` 및 `20260902120000_fix_review_receive_detail_photo_preview.sql`을 적용했다. 후자의 migration들은 기존 RPC를 유지하고 v2 RPC·index·permission pair·batch·photo preview 계약을 additive로 추가한다. 공유 Kong CORS의 wildcard 응답을 피하기 위해 self-hosted 웹은 `/api/admin-gateway` same-origin proxy를 사용한다. 저장소 기본값은 `READY=false`이며, 대응 RPC가 없으면 Edge Function은 `ADMIN_SCHEMA_NOT_READY`를 반환한다.
...
| `review_receive.list` | `get_admin_review_receive_product_summaries_gateway_v2` | `product.read`, `submission.read` |
| `review_receive.detail` | `get_admin_review_receive_detail_v2` | `product.read`, `submission.read` |
...
| `review_receive.photos` | `get_admin_evidence_photos_v2` | `submission.photo.read` |
...
| `product_overview.list` | `get_admin_product_overview_rows_gateway_v2` | `product.read`, `submission.read` |
| `product_overview.submissions.delete` | `delete_admin_product_overview_submissions` | `submission.delete`, `submission.photo.delete` |
| `dashboard.read` | `get_admin_dashboard_data_v2` | `menu.dashboard` + RPC resource 범위 |
| `export.read` | `get_admin_export_data` | `export.execute` |
| `export.photos.read` | `get_admin_photo_export_data_v2` | `export.execute`, `submission.photo.read` |
| `file_upload.apply` | `apply_admin_file_upload` | `product.create`, `submission.create`, `submission.update` |
| `bulk_edit.rows` | `get_admin_bulk_edit_rows_v2` | `bulk_edit.execute`, `submission.read` |
...
위 함수들은 모두 `security definer`, 고정 `search_path = pg_catalog, public`, `service_role` 전용 execute 권한을 사용하도록 초안에 기록되어 있다. staging에서 SQL 구문·권한·반환 결과를 검증했고, production에는 bundle 후속 RPC와 2026-09-01~02의 scope/filter/batch/photo-preview 후속 RPC를 적용했다. 운영 RLS 자체는 이번 범위에서 변경하지 않았다.
...
상품·권한·성능 3건 후속 계약은 `20260901120000_fix_admin_scope_filters_photo_bulk.sql`과 `20260902120000_fix_review_receive_detail_photo_preview.sql`에 정의한다. 상품전체보기 v2는 UI 열 전체를 서버 필터로 처리하고 `purchase_account`, `deposited_at`, 숫자·boolean·사진 존재 필터를 페이지 이전에 적용한다. 공통 scope는 권한 binding의 최대 범위와 화면의 개인/회사 요청 범위를 분리하며, `product.read`·`submission.read` 범위는 설정에서 쌍으로 저장한다. 리뷰받기 상세의 첫 응답은 submission별 대표 사진만 반환하고, 사진 클릭 시 전체 사진을 조회한다. 상품/리뷰어 일괄입력은 기존 insert·중복·부분 성공 의미를 유지한 단일 batch RPC를 사용한다. 두 migration은 production에서 단일 transaction으로 적용했고, 신규 함수의 `service_role=true`, `authenticated=false`, `anon=false`를 확인했다.
...
기존 direct Supabase 경로는 canary와 rollback 검증이 끝날 때까지 보존한다. staging migration/RPC/read/export 계약과 `admin-gateway` 런타임 경계, same-origin 웹 proxy 이미지의 비인증 smoke 및 이전 hyejin2054 인증 read/write canary는 완료했고, Kong wildcard CORS 때문에 직접 cross-origin gateway 호출은 사용하지 않는다. production에는 bundle 계약과 이번 scope/filter/batch/photo-preview 후속 계약 및 해당 웹 수정이 반영됐다. 이번 후속 배포의 실제 제한 계정 read/write·100건 업무 canary는 사용자의 로그인 세션으로 수행해야 한다. 문제 발생 시 `READY=false`로 되돌려 클라이언트의 legacy 경로를 복구하고, 운영 DB/RPC를 임의로 삭제하지 않는다.
