...
tests/
  ...
  adminAccessControl.test.js / adminCapabilities.test.js
                          관리자 메뉴 경계·scope capability fallback 계약 테스트
  reviewReceiveBulkInput.test.js / reviewReceiveDetailTable.test.js
                          구매정보 parser·리뷰받기 필터 정규화·날짜 범위 테스트
...
- 상품 상세·삭제·사진 흐름 전체에는 아직 통합 테스트가 부족해 일부 리팩터링 안정성이 빌드와 수동 확인에 의존합니다.
...
- Node 내장 테스트 41개로 페이지 조회·bulk Excel·대시보드 집계·메뉴/권한·입력 parser·리뷰받기 필터·Supabase allowlist를 검증합니다.
- 상품 상세 화면의 사진/삭제/단계 전환과 실제 네트워크 오류 경로는 여전히 브라우저 smoke와 수동 확인 의존도가 높습니다.
...
- 서비스는 분리됐고 순수 경계 테스트가 추가됐지만, 도메인 전체 흐름을 검증하는 브라우저/통합 테스트 인프라는 아직 제한적입니다.
