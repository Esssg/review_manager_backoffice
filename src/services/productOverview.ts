...
import { PRODUCT_OVERVIEW_COLUMNS, sortProductOverviewRows } from "@/utils/productOverviewRows";
...
function buildProductOverviewPageResult(adminId, options, result, pageSize, scopeOverride = null) {
...
export async function fetchAdminProductOverview(adminId, options = {}) {
  const sort = normalizeSortState(options.sort, getProductOverviewSortKeys());
  ...
  if (isAdminGatewayConfigured()) {
    ...
    const result = await callAdminGatewayOperation(
      isSortedRequest
        ? ADMIN_GATEWAY_OPERATION.PRODUCT_OVERVIEW_LIST_SORTED
        : ADMIN_GATEWAY_OPERATION.PRODUCT_OVERVIEW_LIST,
      {
        ...
        p_force_personal_scope: options.scopePolicy === "personal",
    ...
    return buildProductOverviewPageResult(adminId, options, result, pageSize);
  }
  ...
  if (isSortedRequest) {
    const pageResult = buildProductOverviewPageResult(adminId, options, result, pageSize, scope);
    const sortedRows = sortProductOverviewRows(pageResult.rowsResult.data ?? [], sort);

    return {
      ...pageResult,
      rowsResult: {
        ...pageResult.rowsResult,
        data: sortedRows
      },
      submissionsResult: {
        ...pageResult.submissionsResult,
        data: sortedRows
      }
    };
  }
