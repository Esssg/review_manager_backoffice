...
export default function AdminProductOverviewPage({ viewMode = "all" }) {
  ...
  const canDeleteSubmission = Boolean(permissions[ADMIN_PERMISSION_CODE.SUBMISSION_DELETE]?.allowed);
  ...
  const canDeleteOverviewRows = canDeleteSubmission && canDeletePhoto;
  ...
  useEffect(() => {
    ...
    const loadOverview = async () => {
      ...
      if (!canReadProduct || !canReadSubmission) {
        ...
        setErrorMessage("상품전체보기 상품과 제출 조회 권한이 필요합니다.");
        setIsLoading(false);
        return;
      }
  ...
  const loadAllPhotoNavigationRows = useCallback(async () => {
    if (
      photoNavigationLoadedRef.current ||
  ...
  const handlePurchaseBulkApply = async () => {
    ...
    if (!canUpdateSubmission) {
      setPurchaseBulkFeedback("구매정보를 수정할 권한이 없습니다.", "error");
      return;
    }
    ...
    for (let index = 0; index < purchaseBulkPreview.parsedEntries.length; index += 1) {
      ...
      const result = await updateReviewReceiveSubmission(targetRow.submission_id, payload);
  ...
  const handlePurchaseAssignApply = async () => {
    ...
    if (!canCreateSubmission && !canUpdateSubmission) {
      setPurchaseAssignFeedback("제출 데이터를 생성하거나 수정할 권한이 없습니다.", "error");
      return;
    }
    ...
    const hasCreateTargets = purchaseAssignPreview.entries.some((entry) => {
      const targetRow =
    ...
    const hasUpdateTargets = purchaseAssignPreview.entries.some((entry) => {
      const targetRow =
        purchaseAssignMode !== "append" && entry.row_number != null
          ? purchaseAssignPositionMaps.rowByNumberMap[entry.row_number] ?? null
          : null;
    ...
    if ((hasCreateTargets && !canCreateSubmission) || (hasUpdateTargets && !canUpdateSubmission)) {
      setPurchaseAssignFeedback(
        hasCreateTargets && !canCreateSubmission
          ? "새 제출 데이터를 생성할 권한이 없습니다."
          : "기존 제출 데이터를 수정할 권한이 없습니다.",
        "error"
      );
      return;
    }
  ...
  const reviewBatchEnterConfirm = useModalEnterConfirm({
    ...
    isDisabled:
      !canVerifyDeposit || !canUpdateSubmission || !canUpdateDepositorName || isApplyingReviewBatch || reviewBatchTargetRows.length === 0,
  ...
  const renderPurchaseActions = (scopeKey) => (
    <div className="review-receive-toolbar-actions">
      <Button
        ...
        disabled={(scopeRowsByKey[scopeKey] ?? []).length === 0 || (!canCreateSubmission && !canUpdateSubmission)}
      >
    ...
    return (
      <div className="review-receive-toolbar-actions">
        <Button
          ...
          disabled={!canVerifyDeposit || !canUpdateSubmission || !canUpdateDepositorName || isDepositVerifyDisabled}
        >
    ...
    return (
      <div className="review-receive-toolbar-actions">
        <Button
          ...
          disabled={!canUpdateSubmission || !canApplyReviewVerify || isApplyingReviewVerify}
        >
    ...
    return (
      <div className="review-receive-toolbar-actions product-overview-reset-actions">
        <Button
          ...
          disabled={!canExport || exportRows.length === 0}
        >
          엑셀로 내보내기
        </Button>
        {shouldShowDepositCancelButton && (
          <Button
            ...
            disabled={!canVerifyDeposit || !canUpdateSubmission || !canUpdateDepositorName || !canCancelSelectedDeposit || isCancellingDeposit}
          >
            입금완료 취소하기
          </Button>
        )}
        ...
        <Button
          ...
          disabled={!canDeleteOverviewRows || !hasSelectedRows || isDeletingSelectedRows}
        >
          선택 행 삭제하기
        </Button>
      </div>
  ...
  return (
    <>
      <header className="admin-header admin-scope-page-header admin-product-overview-page-header">
        <div className="admin-page-heading">
          <span className="admin-page-eyebrow">데이터 운영</span>
          <h1>상품전체보기</h1>
          <p>{currentViewHeaderText}</p>
        </div>
        <div className="admin-page-header-tools admin-page-header-tools-solo" aria-label="상품전체보기 화면 도구">
          <AdminScopeCard
            adminId={adminId}
      ...
      {isStatusView && (
        <section className="dashboard-panel product-overview-toolbar-panel" aria-label="상품전체보기 상태 선택">
      ...
      <Button
        ...
        className="admin-primary-button"
                ...
                disabled={
                  !canUpdateSubmission ||
                  isApplyingPurchaseBulk ||
                  purchaseBulkPreview.status !== "ready" ||
      ...
      {exportModal.isOpen && (
              ...
              <div className="product-overview-export-column-panel">
                <label className="pretty-checkbox product-overview-export-select-all">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleToggleAllExportColumns(Boolean(checked))}
                    aria-label="전체 선택하기"
                  />
                  <span>전체 선택하기</span>
                </label>
                  ...
                  {PRODUCT_OVERVIEW_EXPORT_COLUMNS.map((column) => (
                    <label key={column.key} className="pretty-checkbox product-overview-export-column-option">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => handleToggleExportColumn(column.key, Boolean(checked))}
        ...
        <Button
          type="button"
                ...
                disabled={
                  !canExport ||
                  isExportingProductOverview ||
                  exportColumnKeys.length === 0 ||
                  exportModalRows.length === 0
                }
              >
          ...
          >
              ...
              <div className="review-receive-bulk-fields">
                {purchaseAssignVisibleProducts.length > 1 && (
                  <Select
                    ...
                    onValueChange={(value) => {
                      setPurchaseAssignTargetProductId(value);
                      setPurchaseAssignMessage("");
                    }}
                    disabled={isApplyingPurchaseAssign}
                  >
                    <SelectTrigger className="table-cell-input" aria-label="구매자 일괄 입력 대상 상품">
            ...
            <div className="review-receive-modal-actions">
              ...
              <Button
                ...
                disabled={
                  (!canCreateSubmission && !canUpdateSubmission) ||
                  isApplyingPurchaseAssign ||
                  Boolean(purchaseAssignPreview.errorMessage) ||
                  purchaseAssignPreview.entries.length === 0
                }
                ...
                <div className="review-receive-review-batch-grid">
                  ...
                  <div className="detail-summary-item">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => {
                        setReviewBatchActualDepositorMode(value);
                        setReviewBatchMessage("");
                      }}
                      disabled={isApplyingReviewBatch}
                    >
                      <SelectTrigger className="table-cell-input" aria-label="실제입금자명 처리">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="planned">예정 입금자명으로 처리하기</SelectItem>
                        <SelectItem value="custom">직접입력하기</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
