    ...
    >
      ...
      <section className="export-toolbar" aria-label="사진 내보내기 요약">
        ...
        <div className="export-toolbar-actions">
          <Button type="button" variant="outline" className="admin-secondary-button" onClick={resetFilters} disabled={isLoading}>
            필터 초기화
          </Button>
          <Button
            type="button"
        ...
        <div className="export-preview-table-wrap export-photo-product-table-wrap">
          <Table className="export-preview-table export-photo-product-table">
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={PHOTO_EXPORT_FILTER_COLUMNS.length}>상품내역을 불러오는 중입니다.</TableCell>
                </TableRow>
              ) : filteredRows.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={PHOTO_EXPORT_FILTER_COLUMNS.length}>필터 조건에 맞는 상품이 없습니다.</TableCell>
                </TableRow>
              ) : (
                filteredRows.map((row) => (
                  <TableRow key={row.id}>
                    {PHOTO_EXPORT_FILTER_COLUMNS.map((column) => (
                      <TableCell key={`${row.id}-${column.key}`}>{row[column.key] == null || row[column.key] === "" ? "-" : row[column.key]}</TableCell>
                    ))}
                  </TableRow>
                ))
              )}
