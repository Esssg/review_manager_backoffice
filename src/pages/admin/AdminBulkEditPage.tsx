...
export default function AdminBulkEditPage() {
  ...
  return (
    <>
      <header className="admin-header admin-scope-page-header admin-bulk-edit-page-header">
        <div className="admin-page-heading">
          <span className="admin-page-eyebrow">데이터 수정</span>
          <h1>일괄수정하기</h1>
          <p>상품전체보기 데이터를 Excel로 수정하고 변경 내용을 확인한 뒤 적용합니다.</p>
        </div>
        <div className="admin-page-header-tools admin-page-header-tools-solo" aria-label="일괄수정하기 화면 도구">
          <AdminScopeCard
            adminId={adminId}
            ...
            {modalStep === "upload" ? (
              ...
              <div className="review-receive-modal-body bulk-edit-modal-body">
                ...
                <div className="table-scroll-wrap bulk-edit-preview-wrap">
                  <Table className="review-receive-table bulk-edit-preview-table"><colgroup><col className="bulk-edit-col-row" /><col className="bulk-edit-col-id" /><col className="bulk-edit-col-context" /><col className="bulk-edit-col-field" /><col className="bulk-edit-col-value" /><col className="bulk-edit-col-value" /></colgroup><TableHeader><TableRow><TableHead>Excel 행</TableHead><TableHead>제출 ID</TableHead><TableHead>식별 정보</TableHead><TableHead>수정 항목</TableHead><TableHead>기존 값</TableHead><TableHead>변경 값</TableHead></TableRow></TableHeader><TableBody>
                    {uploadResult.changes.flatMap((change) => change.fields.map((field, index) => <TableRow key={`${change.submissionId}-${field.key}`}><TableCell>{index === 0 ? change.rowNumber : ""}</TableCell><TableCell>{index === 0 ? change.submissionId : ""}</TableCell><TableCell>{index === 0 ? [change.assignName, change.orderNumber].filter(Boolean).join(" / ") || "-" : ""}</TableCell><TableCell>{field.label}</TableCell><TableCell><BulkEditPreviewValue value={field.previousValue} column={field.column} /></TableCell><TableCell><BulkEditPreviewValue value={field.nextValue} column={field.column} /></TableCell></TableRow>))}
                  </TableBody></Table>
                </div>
              </div>
            )}
