...
import { buildFileUploadFailureResult } from "@/utils/fileUploadPersistence";
            ...
            <TableBody>
              {rows.map((issue, index) => (
                <Fragment key={`${issue.code}-${issue.rowNumber}-${issue.column}-${index}`}>
                  ...
                  {rowDataByNumber && (
                    <TableRow className="file-upload-issue-detail-row">
                      <TableCell colSpan={4}>
...
export default function AdminFileUploadPage() {
  ...
  const parseSelectedFile = async (file) => {
    ...
    try {
      const { parseFileUploadFile } = await import("@/utils/fileUploadExcel");
    ...
    <div className="file-upload-page" onPaste={handlePaste}>
      <header className="admin-header">
        ...
        <div className="admin-header-actions">
          <Button type="button" variant="outline" className="admin-secondary-button" onClick={handleDownloadTemplate}>
            샘플 양식 다운로드
          </Button>
      ...
      <section className="dashboard-panel file-upload-drop-panel" aria-label="파일 선택">
        ...
        >
          ...
          <div className="file-upload-actions">
            ...
            />
            <Button type="button" variant="outline" className="admin-secondary-button" onClick={() => fileInputRef.current?.click()}>
              파일 찾기
            </Button>
            <Button type="button" variant="link" className="admin-link-button" onClick={resetFileState} disabled={isParsing || !parseResult}>
              초기화
            </Button>
          </div>
        </div>
            ...
            {previewRows.length === 0 ? (
              ...
              <div className="table-scroll-wrap file-upload-table-wrap">
                <Table className="file-upload-table">
                  ...
                  <TableBody>
                    {previewRows.map((row) => (
                      <TableRow key={`${row.clientProductKey}-${row.sourceRowNumber}-${row.payload.order_number}`}>
                        <TableCell>{row.sourceRowNumber}</TableCell>
                        <TableCell>{uploadableProductTitleByKey.get(row.clientProductKey) ?? productTitleByKey.get(row.clientProductKey) ?? "-"}</TableCell>
                        <TableCell>{row.payload.order_number ?? "-"}</TableCell>
                        <TableCell>{row.payload.buyer_name ?? "-"}</TableCell>
                        <TableCell>{row.payload.recipient_name ?? "-"}</TableCell>
                        <TableCell>
                          {[row.payload.bank_name, row.payload.bank_account, row.payload.account_holder]
                            .filter(Boolean)
                            .join(" / ") || "-"}
                        </TableCell>
                        <TableCell>{formatNumber(row.payload.amount)}</TableCell>
                        <TableCell>{formatNumber(row.payload.review_fee)}</TableCell>
                        <TableCell>{formatBoolean(row.payload.is_review_verified)}</TableCell>
                        <TableCell>{formatBoolean(row.payload.is_deposit_verified)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
