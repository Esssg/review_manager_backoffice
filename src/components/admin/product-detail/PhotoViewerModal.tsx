...
export default function PhotoViewerModal({ photoViewer, onClose, onNext, onPrev, onRequestDelete = undefined, isDeleting = false }) {
  const handleClose = () => {
    if (!isDeleting) {
      onClose();
    }
    ...
    <div className="photo-modal-backdrop" role="presentation" {...backdropDismissProps}>
      <div className="photo-modal-content" onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
        <div className="photo-modal-actions">
          ...
          <Button type="button" variant="outline" className="photo-modal-close" onClick={handleClose} disabled={isDeleting}>
            닫기
          </Button>
        </div>
        {showNavigation && (
          <Button type="button" variant="ghost" size="icon" className="photo-modal-arrow left" onClick={onPrev} aria-label="이전 사진" disabled={isDeleting}>
            ‹
          </Button>
        )}
        <img
        ...
        />
        {showNavigation && (
          <Button type="button" variant="ghost" size="icon" className="photo-modal-arrow right" onClick={onNext} aria-label="다음 사진" disabled={isDeleting}>
            ›
          </Button>
        )}
      </div>
